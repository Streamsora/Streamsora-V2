...
export const useAniList = (session) => {
  ...
  Page(perPage: 8) {
    ...
    `;
    return await fetchGraphQL(searchQuery, { search, type, isAdult });
  };
...
`;
    return await fetchGraphQL(searchQuery, { search });
  };
  ...
  const getUserLists = async (id) => {
    ...
    `;
    return await fetchGraphQL(getLists, { id });
  };
  ...
  `;
    ...
    if (media && media.type !== "MANGA") {
      ...
      if (customList === true && !lists?.includes("Watched using Streamsora")) {
        lists.push("Watched using Streamsora");
      }
